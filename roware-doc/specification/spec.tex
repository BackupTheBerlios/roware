...
\usecase{Der Nutzer ist als AKT Admin angemeldet. Es existiert eine veröffentlichte Ausschreibung, zu der schon Meldedaten eingegeben worden sind.}
{Der Nutzer bekommt in einer Liste die Aktiven angezeigt, die ein Jugendlizenz vorlegen müssen. Für die jeweilige Veranstaltung (Ausschreibung) hat jeder Aktive ein Attribut JL, das anzeigt, ob eine gültige Jugendlizenz vorliegt, oder nicht. Per Klick kann der Nutzer einen automatischen Abgleich zwischen der Liste der zu kontrollierenden und den vorhandenen Jugendlizenzdaten vornehmen. Hierbei wird auf Gleichheit von Verein, Name und Geburtsjahr getestet. Ist eine entsprechende Lizenz vorhanden, wird das Attribut auf \texttt{true} gesetzt und der jeweilige Aktive taucht nicht mehr in der Liste der zu überprüfenden Personen auf. Wird eine bestimmte Peron ausgewählt (weil sie z.B. nicht in der Liste der Jugendlizenzen enthalten ist), so kann der Nutzer per Klick das JL-Attribut auf \texttt{true} setzen (z.B. bei Vorlage der JL). Hierbei werden die entsprechenden Daten des Aktiven auch in die Liste der JL Daten übernommen um bei späteren Veranstaltungen eine erneute manuelle Überprüfung zu vermeiden.}
{Aktive deren Daten in der JL-Liste enthalten sind werden nicht mehr angezeigt. Aktive, bei denen dies manuell bestätigt wurde werden in die Liste übernommen.}
{Die Liste der uz überprüfenden Aktiven ändert sich nicht.}
{AKT Admin}
...
{Das Gewicht wird für den jeweiligen Aktiven gespeichert.\footnote{Sollte hier ein mehrfaches Editieren während eine Veranstaltung erlaubt sein, oder nicht??}}
...
{Der Nutzer bekommt eine Liste aller abgeschlossenen Rennen angezeigt. Das sind die Rennen, in denen für alle Starter ein Ergebnis (entweder eine Zeit oder abgemeldet oder gekentert) vermerkt ist. Wählt der Nutzer ein bestimmtes Rennen aus, bekommt er die Rangfolge der gestarteten Mannschaften angezeigt. Diese wird vom System erzeugt. Hierbei findet die Sortierung in folgender Weise statt: Zeit -> gekentert -> Abmeldung. Innerhalb der letzten beiden Gruppen wird absteigend nach Startnummer sortiert. Für aus dieser Liste kann der Nutzer nun Mannschaften auswählen, für die Urkunden gedruckt werden sollen. Des weiteren kann der Nutzer ein Urkunden-Template auswählen, in das die Urkunden gedruckt werden sollen. Als Templates dienen hierbei HTML-Dateien, in die bestimmte Token eingebettet werden können (\#NAME, \#ZEIT, \#PLATZ usw.). Diese werden vom System durch die jeweiligen Daten der Mannschaft ersetzt. Solche HTML-Templates sind auch durch den Nutzer leicht zu erstellen und somit an seine Bedürfnisse anzupassen. Berücksichtigt werden alle Dateien in einem bestimmten Template-Verzeichnis, die möglichen Token werden vom \roware vorgegeben. Das System zeigt nun die Seiten mit den eingesetzten Daten an, so daß der Nutzer diese über die Druckfunktion des Browsers drucken kann. \footnote{Interessant wäre hier sicherlich auch PDF-Erzeugung, gerade wenn mehrere Seiten (Urkunden) auf einmal ausgegeben werden sollen}}
...
{Sind alle Rennen einer Veranstaltung abgeschlossen,d.h. für alle Starter ist ein Ergebnis (entweder eine Zeit oder abgemeldet oder gekentert) vermerkt oder ein Rennen ist als 'ausgefallen' markiert, kann der Nutzer das Ergebnis der Veranstaltung veröffentlichen. Hierbei wird nach Rennen sortiert die Rangfolge der gestarteten Mannschaften aufgeführt. Diese wird vom System erzeugt. Hierbei findet die Sortierung in folgender Weise statt: Zeit -> gekentert -> Abmeldung. Innerhalb der letzten beiden Gruppen wird absteigend nach Startnummer sortiert. Des weiteren werden der Name und das Datum der Veranstaltung aufgeführt. Dann exportiert das System die relevanten Daten in verschiedene vom Nutzer wählbare Formate. Als Exportformate werden hierbei PDF, HTML und xregatta unterstützt. Diese Daten werden auch auf dem Server gehalten und können durch den Nutzer mittels des Browsers heruntergeladen werden. } 
